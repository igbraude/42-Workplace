<?php
    ...
    function notifOn() {
        include("../config/database-setup.php");
        $query = sprintf('SELECT `notif` FROM `user` WHERE `username` = "%s"', $_SESSION['username']);
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $result = $stmt->fetch(PDO::FETCH_BOTH);
        if ($result['notif'] == 'Y') {
    ...
    function updateNotifOn() {
        include("../config/database-setup.php");
        $query = sprintf('UPDATE `user` SET `notif` = "Y" WHERE `username` = "%s"', $_SESSION['username']);
        $stmt = $conn->prepare($query);
        $stmt->execute();
    }

    function updateNotifOff() {
        include("../config/database-setup.php");
        $query = sprintf('UPDATE `user` SET `notif` = "N" WHERE `username` = "%s"', $_SESSION['username']);
        $stmt = $conn->prepare($query);
        $stmt->execute();
    }
