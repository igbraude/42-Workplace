...
function addInHistoriqueDatabase() {
    include("../config/database-setup.php");
    $query = "INSERT INTO `historique` (`path`, `username`)
    VALUE ( ?, ? )";
    $stmt = $conn->prepare($query);
    $stmt->execute(array($_POST['newPicture'], $_SESSION['username']));
}

function addInHistoriqueDatabaseUpload() {
    include("../config/database-setup.php");
    $query = "INSERT INTO `historique` (`path`, `username`)
    VALUE ( ?, ? )";
    $stmt = $conn->prepare($query);
    $stmt->execute(array($_POST['uploadedImage'], $_SESSION['username']));
}

function delete_historique() {
    include("../config/database-setup.php");
    $query = sprintf("DELETE FROM `historique` WHERE `username` = '%s'", $_SESSION['username']);
    $stmt = $conn->prepare($query);
    $stmt->execute();
}
