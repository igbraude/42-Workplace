...
function getImages() {
    ...
    $query = sprintf('SELECT * FROM `historique` WHERE `username`="%s"', $_SESSION['username']);
    foreach($conn->query($query) as $row) {
        $img = new Images(0, $row['username'], $row['path'], 0, 0, 'N');
        if ($imgObject == null) {
            $imgObject = array($img);
        }
        else  {
...
<form class="img" enctype="multipart/form-data" method="post">
    <div id="my_camera">
      ...
      <input type="submit" id="takePicButton" <?php if ($disabled != null) { echo $disabled; } ?> action="takePhoto" value="Take Photo">
